...
export default function DashboardHome() {
  // Dropdown state for mobile user menu
          ...
          <div className="md:hidden relative">
            ...
            {dropdownOpen && (
              <div ref={dropdownRef} className={`absolute right-0 mt-2 w-48 bg-white dark:bg-[#1e3a34] rounded-xl shadow-2xl border border-[#99FF77]/30 dark:border-[#66FF99]/20 z-50 p-3 flex flex-col gap-2 animate-fade-in`}> 
                <div className="flex flex-col items-center gap-1 pb-2 border-b border-[#99FF77]/20 dark:border-[#66FF99]/10">
                  <div className={`w-12 h-12 rounded-full overflow-hidden border-2 ${theme === 'light' ? 'border-[#99FF77]' : 'border-[#66FF99]'} shadow bg-white/10 flex items-center justify-center`}>
                    {user && user.profilePic ? (
                      <img src={user.profilePic} alt="User Avatar" className="w-full h-full object-cover" />
                    ) : (
                      <span className={`text-xl font-bold ${theme === 'light' ? 'text-[#1e1a2b]/50' : 'text-white/50'}`}>{user?.name?.[0] || "U"}</span>
                    )}
                  </div>
                  <span className={`text-sm font-semibold leading-tight truncate max-w-[120px] ${theme === 'light' ? 'text-[#1e1a2b]' : 'text-white'}`}>{user?.name || "User"}</span>
                  <span className={`text-xs leading-tight truncate max-w-[120px] ${theme === 'light' ? 'text-[#1e1a2b]/50' : 'text-white/50'}`}>{user?.email || "user@email.com"}</span>
                </div>
                <button className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#99FF77]/10 dark:hover:bg-[#66FF99]/10 transition text-left`}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a3 3 0 003.22 0L22 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                  Mail
                </button>
                <button className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#99FF77]/10 dark:hover:bg-[#66FF99]/10 transition text-left`}>
                ...
                <button className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#99FF77]/10 dark:hover:bg-[#66FF99]/10 transition text-left`} onClick={toggleTheme}>
                  {theme === 'dark' ? (
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><circle cx="12" cy="12" r="5" stroke="currentColor" strokeWidth="2" /><path stroke="currentColor" strokeWidth="2" strokeLinecap="round" d="M12 1v2m0 18v2m11-11h-2M3 12H1m16.95 7.07l-1.41-1.41M6.34 6.34L4.93 4.93m12.02 0l-1.41 1.41M6.34 17.66l-1.41 1.41" /></svg>
                  ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12.79A9 9 0 1111.21 3a7 7 0 109.79 9.79z" /></svg>
                ...
                <button className={`w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-[#FF7777]/10 dark:hover:bg-[#FF6666]/10 transition text-left`} onClick={() => { localStorage.removeItem('token'); window.location.href = '/'; }}>
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-[#FF7777] dark:text-[#FF6666]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 11-6 0v-1m6 0H9" /></svg>
                  Logout
                </button>
              </div>
            )}
